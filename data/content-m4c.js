...
sequenceDiagram
  ...
  `,
  quiz: [
    {q: "A che livello dello stack TCP/IP opera ICMP?", a: ["Applicativo (come HTTP, perché ping è un comando da shell)","Trasporto (come TCP/UDP, è incapsulato in IP)","Stesso livello di IP (rete): ICMP è incapsulato direttamente in IP, non in TCP/UDP","Datalink"], correct: 2, explain: "ICMP viaggia direttamente nel campo Protocol di IPv4 (=1) o nel Next Header di IPv6 (=58). Non usa TCP/UDP. Anche se il comando ping è un'applicazione, il PROTOCOLLO ICMP è di livello 3 (rete), parte integrante dello stack IP."},
    ...
    {q: "Cosa contiene il payload di un messaggio ICMP di errore (es. Destination Unreachable)?", a: ["Solo il messaggio testuale dell'errore","L'intero pacchetto IP scartato (header + tutto il payload)","L'header IP del pacchetto scartato + i primi 8 byte del suo payload","Solo l'IP mittente del pacchetto scartato"], correct: 2, explain: "Per consentire al mittente di identificare la connessione/processo coinvolto: l'header IP completo + i primi 8 byte del payload (che per TCP/UDP contengono le porte sorgente e destinazione). Trasportare l'intero pacchetto sarebbe troppo costoso e farebbe esplodere ICMP storms."}
  ]
};
