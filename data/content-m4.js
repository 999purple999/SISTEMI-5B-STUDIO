...
window.CHAPTERS.m4c1 = {
  ...
  quiz: [
    ...
    {q: "Nel cablaggio strutturato secondo EIA/TIA 568, cosa è il FD (Floor Distributor)?", a: ["Il PC dell'utente collegato alla presa","L'armadio di piano dove termina la dorsale verticale e da cui parte il cablaggio orizzontale verso le prese","Un router di livello 3","Un firewall hardware"], correct: 1, explain: "FD (Floor Distributor) è l'armadio di piano: collega la dorsale verticale (BD-FD) al cablaggio orizzontale (FD-TO) verso le prese utente. La gerarchia è CD (campus) → BD (edificio) → FD (piano) → TO (presa)."},
    {q: "Cat 6 supporta 10 Gbps ma con un limite di distanza. Cat 6A invece supporta 10 Gbps fino a 100 m. Perché?", a: ["Cat 6A ha più conduttori","Cat 6A ha banda passante 500 MHz (vs 250 MHz di Cat 6) e schermatura migliore: regge la diafonia su distanze lunghe","Cat 6A è in fibra ottica","Sono identici"], correct: 1, explain: "Cat 6 a 10 Gbps regge solo ~37-55 m per via di NEXT/AXT (diafonia). Cat 6A (250→500 MHz, schermatura per coppia) supporta 10G/100m. Cat 7/8 vanno oltre."},
    {q: "Quale combinazione fibra/transceiver è ottimale per una dorsale di 300 m tra due edifici a 10 Gbit/s?", a: ["Cat 6A su 10GBASE-T","Fibra OM3 multimodale + transceiver SFP+ 10GBASE-SR","Fibra OS2 monomodale + transceiver 100GBASE-SR","Wireless 802.11ac"], correct: 1, explain: "10GBASE-T è limitato a 100m. 10GBASE-SR (multimodale, sorgente VCSEL 850nm) raggiunge 300-400m su OM3, 550m su OM4: economico e perfetto per dorsali inter-edificio brevi. OS2 monomodale serve per distanze >2km."},
    {q: "Differenza tra UTP, FTP e STP?", a: ["UTP non ha schermatura; FTP ha un foglio metallico globale; STP ha schermo per ogni coppia + foglio globale","UTP è in fibra; FTP e STP in rame","Sono tre nomi dello stesso cavo","UTP ha 4 coppie, FTP ne ha 2, STP ne ha 8"], correct: 0, explain: "UTP=Unshielded Twisted Pair (no schermatura, più economico, più sensibile a EMI). FTP=Foiled TP (foglio globale). STP=Shielded TP (schermo per coppia + globale, massima resistenza alle interferenze, più rigido e costoso)."}
  ]
};
...
window.CHAPTERS.m4c4 = {
  ...
  `,
  quiz: [
    {q: "Quale è il PRINCIPALE svantaggio del FLSM rispetto al VLSM?", a: ["FLSM è più complesso da configurare","FLSM spreca indirizzi quando le sottoreti hanno fabbisogni di host molto diversi","FLSM non funziona con CIDR","FLSM richiede router più costosi"], correct: 1, explain: "Con FLSM tutte le sottoreti hanno la stessa maschera. Se ho 3 reparti con 100/10/2 host, FLSM mi obbligherebbe a usare /25 per tutti (uno spreco enorme per il link a 2 host che riceverebbe 126 IP utili). VLSM permette /25, /28 e /30 rispettivamente."},
    {q: "Per usare il VLSM, quale tipo di protocollo di routing serve?", a: ["Solo classful routing (es. RIPv1)","Classless routing (RIPv2, OSPF, EIGRP, IS-IS) che propaga la subnet mask negli annunci","Solo routing statico","Solo BGP"], correct: 1, explain: "RIPv1 è classful: non trasmette la subnet mask negli aggiornamenti, quindi non gestisce maschere variabili. RIPv2, OSPF, EIGRP, IS-IS sono classless e supportano VLSM. Anche il routing statico supporta VLSM."},
