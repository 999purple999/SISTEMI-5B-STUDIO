...
Router(config)# end</pre>
  ...
  quiz: [
    ...
    {q: "Per la dorsale 300m tra i due edifici, quale specifica scegliereste?", a: ["10GBASE-T su Cat 6A — il rame è limitato a 100m, non basta per 300m","10GBASE-SR con SFP+ e fibra OM3 multimodale LC duplex","1000BASE-T su Cat 5e","Wireless 802.11ac"], correct: 1, explain: "Cat 6A è limitato a 100m. 10GBASE-SR (multimodale, sorgente VCSEL 850nm, fino a 300-400m su OM3) è la scelta corretta: SFP+ economico, fibra OM3 economica, stabile 10 Gbit/s. Lascia margine futuro vs WAN 1G/300M."},
    {q: "Lo switch del Magazzino deve alimentare un Access Point Wi-Fi 5 con MIMO 2×2 a 80 MHz tramite PoE. Quale standard è dimensionato correttamente?", a: ["802.3af PoE — 15.4 W (insufficiente per AP Wi-Fi 5 multi-MIMO)","802.3at PoE+ — 30 W alla porta (~25.5 W al device, sufficiente per AP Wi-Fi 5/6)","802.3bt PoE++ — 60-100 W (sovradimensionato, costo eccessivo)","Nessun PoE, alimentazione separata dell'AP"], correct: 1, explain: "PoE+ (802.3at) eroga 30 W alla porta, 25.5 W al device: dimensionato per AP Wi-Fi 5/6 con MIMO multi-flusso. PoE 15.4 W basta solo per AP entry-level o IP cam. PoE++ è eccessivo (e più caro) per Wi-Fi 5."},
    {q: "Perché in LogiPack si è scelto WPA2-Enterprise + RADIUS invece di WPA2-Personal (PSK)?", a: ["È più veloce in trasmissione","Permette credenziali per dipendente: tracciabilità per utente, revoca singolo accesso quando un dipendente lascia, niente PSK condivisa da cambiare per tutti","Funziona meglio con i palmari","WPA2-Personal non esiste"], correct: 1, explain: "WPA2-Personal usa una PSK condivisa: se un dipendente lascia, dovresti cambiarla per tutti. WPA2-Enterprise + RADIUS: ogni utente ha credenziali personali, revocabili singolarmente, log per dipendente. Standard nelle aziende."},
    {q: "Il VoIP del reparto Produzione ha un'interfaccia FastEthernet 100BASE-TX. È possibile collegarlo a una porta GE PoE dello switch?", a: ["No, serve una porta a 100 Mbit/s","Sì: la porta GE è retro-compatibile a 100 Mbit/s (auto-negotiation) e fornisce alimentazione PoE 802.3af sufficiente al VoIP","No, PoE non funziona con VoIP","Sì ma serve un convertitore"], correct: 1, explain: "Le porte GE sono auto-negotianti e supportano 10/100/1000 Mbit/s. PoE 802.3af (15.4 W) è dimensionato per VoIP (consumo tipico 5-10 W). Cavo Cat 5e adatto per 100BASE-TX e PoE."}
  ]
};
...
ip nat inside source static udp 192.168.2.30 53  203.0.113.10 53</pre>
  ...
  quiz: [
    ...
    {q: "Perché in Esercizio 1 si applica SNAT al web server della DMZ invece di NAT dinamico?", a: ["SNAT è generalmente più sicuro","SNAT mappa SEMPRE lo stesso IP_priv allo stesso IP_pub: un server pubblico DEVE avere IP fisso (pubblicato in DNS); con NAT dinamico l'IP cambierebbe","SNAT è più veloce di NAT dinamico","Il NAT dinamico non funziona in DMZ"], correct: 1, explain: "Un server pubblico (web, mail, DNS) DEVE essere raggiungibile a un IP costante per essere pubblicato in DNS e essere trovato dai client. NAT dinamico assegna IP pubblici da un pool quando un host esce, l'IP cambia ad ogni traduzione: inutilizzabile per server. SNAT (statica) garantisce mapping permanente IP_priv ↔ IP_pub."},
    {q: "L'ACL sull'interfaccia ESTERNA permette: HTTP/HTTPS verso 198.51.100.50 + 'established' verso la LAN. Perché 'established'?", a: ["Per cifrare il traffico","Per permettere risposte a connessioni iniziate dalla LAN (TCP ACK=1) ma BLOCCARE nuove connessioni iniziate da Internet (TCP SYN=1) verso la LAN","Per testare il firewall","Per nessun motivo"], correct: 1, explain: "L'opzione 'established' permette pacchetti TCP con flag ACK=1: sono risposte a connessioni che la LAN ha aperto verso Internet. Internet NON può aprire nuove connessioni alla LAN (mancano permit per pacchetti SYN). I server in DMZ invece hanno permit espliciti su porte specifiche perché DEVONO ricevere nuove connessioni. È il principio del 'firewall stateful' implementato in modo stateless via flag TCP."},
    {q: "Quale è il limite di sicurezza dell'opzione 'established' in un ACL classico (non stateful)?", a: ["Nessuno","Non protegge da TCP ACK scan: un attaccante può inviare pacchetti con flag ACK=1 forgiati per scoprire host vivi senza fare un vero handshake","È troppo lenta","Funziona solo su UDP"], correct: 1, explain: "TCP ACK scan: l'attaccante invia pacchetti con ACK=1 (che 'established' lascia passare) anche se non c'è una vera connessione iniziata. Un firewall STATEFUL invece tiene traccia delle connessioni reali e scarta gli ACK senza una sessione corrispondente. Per questo serve uno Stateful Packet Inspection firewall."}
  ]
};
